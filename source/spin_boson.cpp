...
spin_boson::spin_boson( double delta_ecoup, vector<double> wbath, vector<double> cbath): d(delta_ecoup), wb(wbath), cb(cbath) {}
double spin_boson::operator() (const vector<double> &s){
  ...
  ecoup*=d;
  bcoup*=0.5*(s[1]*s[1]+s[2]*s[2]-s[3]*s[3]-s[4]*s[4]);
  bham*=0.5;
  
  return bham+bcoup+ecoup;
}

eom_spin_boson::eom_spin_boson( double delta_ecoup, vector<double> wbath, vector<double> cbath): d(delta_ecoup), wb(wbath), cb(cbath) {}
